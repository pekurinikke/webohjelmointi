...
function formatDateTimeFi(date){ return date.toLocaleString("fi-FI",{ weekday:"short", day:"numeric", month:"numeric", hour:"2-digit", minute:"2-digit"}); }
...
function filterEvents(events,startDate,days=7){
...
function updateRangeInfo(events){
  const start = currentStartDate.toLocaleDateString("fi-FI");
  const endDate = new Date(currentStartDate);
  endDate.setDate(endDate.getDate()+DAYS_WINDOW-1);
  const end = endDate.toLocaleDateString("fi-FI");
  rangeTitle.textContent = `${start} – ${end}`;
  eventCount.textContent = `${events.length} tapahtumaa löydetty`;
}
...
async function loadEventsFromApi(){
  try{
    const res = await fetch("https://kide.app/api/v1/events"); // korvaa oikealla API URL
    ...
    allEvents = data.events.map(ev=>({
      ...
      minPrice: ev.min_price,
      maxPrice: ev.max_price,
      availability: ev.remaining_tickets,
      popularity: ev.likes_count||0,
      description: ev.description||""
    }));
  } catch(e){ console.error("Virhe ladattaessa APIa:",e); allEvents=[]; }
}
...
async function init(){
  ...
  prevBtn.addEventListener("click",()=>{
    currentStartDate.setDate(currentStartDate.getDate()-DAYS_WINDOW);
    startDateInput.value=currentStartDate.toISOString().split("T")[0];
    render();
  });
